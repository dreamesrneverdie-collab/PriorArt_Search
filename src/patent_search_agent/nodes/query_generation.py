...
def query_generation_node(state: PatentSearchState, llm: BaseChatModel) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get IPC codes
        ipc_classification = state.get("ipc_classification")
        ...
        ipc_text = ', '.join(ipc_classification)
...
- Include IPC codes using format: IPC:"G06F"
        ...
        return {
            **state,
            ...
            "messages": state.get("messages", []) + [
                HumanMessage(content="Generating search queries for patent databases"),
                response,
                tool_message
            ]
        }
